$(document).ready(function () {
  ...
  $("#del-photo-1").on("click", function(){
    document.getElementById("photo-1-preview").src = "../assets/img/no-image.png";
    $("#photo-1-input").val("");
  })
  //
  $("#del-photo-2").on("click", function(){
    document.getElementById("photo-2-preview").src = "../assets/img/no-image.png";
    $("#photo-2-input").val("");
  })
  //
  $("#del-photo-3").on("click", function(){
    document.getElementById("photo-3-preview").src = "../assets/img/no-image.png";
    $("#photo-3-input").val("");
  })
  ...
  function showProductsData(tableID, page, numRow, sortBy) {
    $.ajax({
  ...
  //
  showProductsData("table-products", 1, 20, "productID asc");
  ...
  $(document).on("click", "#btn-add-product", function(){
    ...
    // Check image
    if($("#photo-1-input").val() == "" && $("#photo-2-input").val() == "" && $("#photo-3-input").val() == ""){
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (result == productSKU) {
            $("#prodSKUAddHelp").text("(!) Đã tồn tại sản phẩm khác có mã SKU này trong gian hàng của bạn, không thể thêm. Vui lòng kiểm tra lại!");
...
$(document).on("click", "#btn-edit-product", function(){
  ...
  // Check image
  if($("[name='isphoto1editempty']").val() == "" && $("[name='isphoto2editempty']").val() == "" && $("[name='isphoto3editempty']").val() == ""){
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (result == newProductSKU && newProductSKU != currProdSKU) {
          $("#prodSKUEditHelp").text("(!) Đã tồn tại sản phẩm khác có mã SKU này trong gian hàng của bạn. Vui lòng kiểm tra lại!");
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            if (result == "success") {
              ...
              $("#alert-success-content").text("Đã cập nhật thông tin kho hàng sản phẩm ");
